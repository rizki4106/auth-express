...
class UserController {
    ...
    static store(req, res) {
        ...
        validasi(req.body, [
        ...
        ]).then((hasil) => __awaiter(this, void 0, void 0, function* () {
            // jika validasi berhasil maka cek apakah email sudah terdaftar atau belum
            const user_lama = yield model.where({ email: req.body.email }).get();
            if (user_lama.length > 0) {
                ...
                // buat hash password
                const pswd = yield bcrypt.hash(req.body.password, 5);
                // enkripsi password
    ...
    static login(req, res) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            validasi(req.body, [
                ...
                const user = yield model.where({ email: email }).get();
                if (user.length > 0) {
                    // jika email terdaftar maka cek password
                    const pswd_hash = user[0].password;
                    const cek_password = yield bcrypt.compare(password, pswd_hash);
                    if (cek_password) {
